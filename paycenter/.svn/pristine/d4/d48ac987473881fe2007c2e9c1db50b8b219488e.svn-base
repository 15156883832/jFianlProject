...
import com.sf.jfinal.qs.core.sms.FlavorBoxPaymentNotifyJob;
...
public class FlavorBoxService {

    private static final Log logger = Log.getLog(FlavorBoxService.class);

    @Before(Tx.class)
    public void handleFlavorBoxNotify(String outTradeNo, Map<String, String> parasMap, boolean isPaySuccess) {
        ...
        if (StringUtils.isNotBlank(outTradeNo) && isPaySuccess) {
            ...
            try {
                PaymentNotifyService.submitWork(new FlavorBoxPaymentNotifyJob(customerName, buyerMobile, totalPurchaseNum.intValue()));
            } catch (Exception ex) {
                logger.error("flavorbox notify failed", ex);
            }
        }
    }
}
