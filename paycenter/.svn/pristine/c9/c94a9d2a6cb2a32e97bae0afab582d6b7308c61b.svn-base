...
import com.sf.jfinal.qs.service.TPNotifyService;
 ...
 * 来电弹屏支付回调处理。
 */
public class TPNotifyController extends Controller {

    private static Log logger = Log.getLog(TPNotifyController.class);

    public void callback() {
        String payType = getPara(0);
        logger.error("[弹屏设备] notify start, payType is: " + payType);
        ...
        if (parser.isSignValid()) {
            ...
            enhance(TPNotifyService.class).handleTPNotify(outTradeNo, parasMap, parser.isSuccess());
        } else {
