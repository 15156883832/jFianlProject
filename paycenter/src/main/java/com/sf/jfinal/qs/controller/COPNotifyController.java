...
import com.sf.jfinal.qs.service.COPNotifyService;
 ...
 * 平台合作商品支付回调处理。
 */
public class COPNotifyController extends Controller {

    private static Log logger = Log.getLog(COPNotifyController.class);

    public void callback() {
        String payType = getPara(0);
        logger.error("[平台合作商品购买] notify start, payType is: " + payType);
        ...
        if (parser.isSignValid()) {
            ...
            enhance(COPNotifyService.class).handleCOPNotify(outTradeNo, parasMap, parser.isSuccess());
        } else {
